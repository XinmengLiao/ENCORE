...
% For full model
if ~geckoLight
    ec.rxnEnzMat = zeros(numel(rxnWithGene),numel(ec.genes)); 
    for r=1:numel(rxnWithGene)
        disp(r);
        rxnGenes   = model.genes(find(model.rxnGeneMat(rxnWithGene(r),:)));
        [~,locEnz] = ismember(rxnGenes,ec.genes);
        if locEnz ~= 0 
            ec.rxnEnzMat(r,locEnz) = 1; 
        end
    end
else
    ec.rxnEnzMat = zeros(numel(ec.rxns),numel(ec.genes)); 
    nextIndex = 1;
    ...
    for i=1:prevNumRxns
        ...
        for j = 1:length(geneNames)
            ...
            if ~isempty(fnd)
                ...
                ec.rxnEnzMat(nextIndex,ismember(ec.genes,andGenes)) = 1; 
            else
                ec.rxnEnzMat(nextIndex,ismember(ec.genes,geneNames(j)))=1;
            end
            nextIndex = nextIndex + 1;
        end
    end
